...
namespace projetJeu
{
    ...
    public class ManetteService : GameComponent, IInputService
    {
        ...
        /// <summary>
        /// Retourne 1.0f si la flèche en haut du pad est pressée, 0.0f sinon.
        /// </summary>
        /// <param name="device">Le numéro de manette à lire (1 à 4).</param>
        /// <returns>Vitesse entre 0.0f et 1.0f.</returns>
        public float DeplacementAvant(int device)
        {
            ...
            // Premièrement vérifier le thumbstick gauche
            if (this.etatManette[device - 1].IsButtonDown(Buttons.LeftThumbstickUp))
            {
                return this.etatManette[device - 1].ThumbSticks.Left.Y;
            }

            // Puisque le thumbstick gauche n'est pas utilisé, essayons le pad
            if (this.etatManette[device - 1].DPad.Up == ButtonState.Pressed)
            {
                return 1.0f;
            }
            else
        ...
        /// <summary>
        /// Retourne 1.0f si la flèche en bas du pad est pressée, 0.0f sinon.
        /// </summary>
        /// <param name="device">Le numéro de manette à lire (1 à 4).</param>
        /// <returns>Vitesse entre 0.0f et 1.0f.</returns>
        public float DeplacementArriere(int device)
        {
            ...
            // Premièrement vérifier le thumbstick gauche
            if (this.etatManette[device - 1].IsButtonDown(Buttons.LeftThumbstickDown))
            {
                return Math.Abs(this.etatManette[device - 1].ThumbSticks.Left.Y);
            }

            // Puisque le thumbstick gauche n'est pas utilisé, essayons le pad
            if (this.etatManette[device - 1].DPad.Down == ButtonState.Pressed)
            {
                return 1.0f;
            }
            else
